...
function classifyEquation(expr) {
  ...
  else if (invTrigCount > 0) score = 40;else if (expCount > 0) score = 35;else if (logCount > 0) score = 30;else if (trigCount > 0) score = 25;else score = Math.max(1, maxDeg) * 10;
...
function physicsStep(ph, explFns, implFns, dt) {
  ...
  for (const {
    ...
    for (let i = 0; i <= SAMPLES; i++) {
      ...
      if (!inDomain(xs, domain)) {
        prevSign = null;
        continue;
      }
      const cy = fn(xs);
      if (!isFinite(cy) || isNaN(cy)) {
        prevSign = null;
        continue;
      }
